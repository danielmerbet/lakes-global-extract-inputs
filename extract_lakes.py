...
for period in get_periods(path):
    ...
    # iterate over lakes
    for lake in lakes[:]:
        print(' ', lake[0],' (lat:'+ lake[1] + ' lon:' + lake[2] + ')')
        ...
        if period.split('_')[0] == first_year:
            lake_data.to_csv(outfile, encoding='utf-8', float_format='%.1f', index=False)
        else:
            lake_data.to_csv(outfile, encoding='utf-8', float_format='%.1f', index=False, mode='a', header=False)
